...
- **텐던 구멍이 막히거나 와이어 저항이 심할 때**: 섬유선 통과 홀 변수인 `tendon_dia = 1.5;` 값을 **1.8 ~ 2.0**으로 넓혀 출력하십시오. 출력 후 1.5mm~2.0mm 드릴 비트로 텐던 경로와 나팔꽃 모양 가이드(Chamfer Cone) 내부를 한 번씩 리밍(Reaming) 가공해주면 다이니마 와이어의 마찰 저항을 줄일 수 있습니다.
