...
> ⚠️ **실전 제조 및 초기 테스트 주의사항 (Prototyping Notice)**
> 사용하시는 3D 프린터의 장비 정밀도와 필라멘트 수축 특성에 따라 관절부가 굳어버리거나(떡짐) 헐거워질 수 있습니다. 
> 전체 출력을 걸기 전에 **특정 손가락 관절 부위만 슬라이서에서 잘라내어 '초기 테스트 버전'을 먼저 사출**해 보시는 것을 권장합니다.
> 
> * **관절이 붙어서 안 움직일 때**: `biocerve_full.scad` 코드의 내부 공차 변수인 `clearance = 0.2;` 값을 **0.3 ~ 0.4**로 조금씩 키우며 수정하십시오.
> * **텐던 구멍이 막힐 때**: 섬유선 통과 홀 변수인 `tendon_dia = 1.5;` 값을 **1.8 ~ 2.0**으로 넓혀 출력하십시오.
