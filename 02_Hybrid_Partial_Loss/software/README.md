...
*   **사멸 격리**: 의수 마디가 장애물로 인해 모터 코일 전류가 8ms 이상 임계치를 초과하면 즉시 사멸 격리.
*   **물리 고정**: 즉시 전력(0) 차단 및 하드웨어 v2.5 설계상의 Y축 정렬 차단 턱/도피 큐브 결착(Wedge-Lock)으로 역하중 지탱.
*   **미러링 소용돌이 우회장 (Vorticity Reroute)**: 잔존 의수 노드들의 생체 미러링 추적 감도 및 스케일링 계수가 동적으로 상향 재조정.
*   **순응성 완성**: 12비트 레졸루션 하드웨어 타이머를 통해 기계와 생체가 물체 모양에 맞게 유기적으로 순응(Compliance).
